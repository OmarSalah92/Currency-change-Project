...
.then((e)=>{
   ...
   mySelect1.addEventListener('change',()=>{
    fetch(`https://v6.exchangerate-api.com/v6/3de26270bde642c2c4e03869/latest/${mySelect1.value}`)
    .then(e=>e.json())
    .then((res)=>{
      ...
      myBtn.addEventListener('click',(n)=>{
        ...
        myDiv2.innerHTML= `<h4> ${myInput1.value*res.conversion_rates[mySelect1.value]} ${mySelect1.value}=${res.conversion_rates[mySelect2.value]*myInput1.value} ${mySelect2.value}</h4>`
      
    })
   })

   mySelect2.addEventListener('change',()=>{
    fetch(`https://v6.exchangerate-api.com/v6/3de26270bde642c2c4e03869/latest/${mySelect2.value} `)
    .then(e=>e.json())
    .then((res)=>{
